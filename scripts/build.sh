...
# copied from: https://github.com/lutris/ffmpeg-nvenc/blob/master/build.sh
...
FDK_AAC_SOURCE="https://github.com/mstorsjo/fdk-aac/zipball/master"
...
InstallDependencies() {
    ...
    sudo apt-get -y install sudo unzip wget git autoconf automake build-essential libass-dev \
...
InstallNvidiaSDK() {
    ...
    sdk_basename="Video_Codec_SDK_${NVIDIA_SDK_VERSION}"
    cd "$source_dir"
...
BuildFdkAac() {
    ...
    cd $source_dir
    wget -4 -O fdk-aac.zip "$FDK_AAC_SOURCE"
    unzip fdk-aac.zip
    cd mstorsjo-fdk-aac*
    autoreconf -fiv
